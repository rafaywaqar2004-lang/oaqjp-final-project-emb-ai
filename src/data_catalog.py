...
_REGISTRY: list[DatasetEntry] = [
    ...
    DatasetEntry(
        name="Chokepoints (map layer)",
        path=Path(CURATED_DIR) / "chokepoints.csv",
        source_type="Manually curated (Wikipedia/Wikidata coordinates + EIA/IMF significance stats)",
        update_cadence="Static -- three well-established physical chokepoints",
        methodology_note="Backs the Chokepoint Exposure Map page. Each row's significance figure (e.g. ~21% of global petroleum liquids through Hormuz) is that chokepoint's own documented global significance, never reprojected onto an individual hub -- see that page's own Methodology section.",
        limitations="Limited to three chokepoints (Strait of Hormuz, Bab-el-Mandeb, Suez Canal); does not model every strait or cable route relevant to the tracked region.",
        country_col=None,
    ),
    DatasetEntry(
        name="Cable Landing Stations (map layer)",
        path=Path(CURATED_DIR) / "cable_landing_stations.csv",
        source_type="Manually curated (submarinenetworks.com / submarinecablemap.com)",
        update_cadence="Static -- one-time research pass",
        methodology_note="Backs the Chokepoint Exposure Map page. A station only appears if a named source explicitly documents its cable systems.",
        limitations="Verified for only 5 of the 12 countries with a tracked AI hub (Saudi Arabia, Qatar, UAE, Oman, Syria) -- the rest show hub-to-chokepoint distance only, with no cable-station estimate.",
        country_col=None,
    ),
    DatasetEntry(
        name="QGIS Hub-Chokepoint Distance Matrix",
        path=Path(COMPUTED_DIR) / "qgis_hub_chokepoint_distances.csv",
        source_type="Computed by real QGIS (PyQGIS QgsDistanceArea, ellipsoidal WGS84) -- not a re-implementation of its math",
        update_cadence="Regenerated via `python src/data_pipeline/generate_qgis_geodata.py` whenever ai_hubs.csv or chokepoints.csv changes",
        methodology_note="Backs the Chokepoint Exposure Map's distance table. Cross-checked against an independent pyproj calculation in tests/test_qgis_geodata.py (agrees to well under 1km on every row). The companion buffer-ring geometries (qgis_chokepoint_buffers.geojson) are generated by the same script but aren't CSV, so aren't listed as a separate catalog row here -- see README.md.",
        limitations="A build-time snapshot, not live-recomputed by the running app -- stale if the underlying hub/chokepoint coordinates change without the generation script being re-run (a test catches drift beyond 1km, but only when the test suite runs).",
        country_col=None,
    ),
    DatasetEntry(
    ...
    DatasetEntry(
        ...
        limitations="24 deals tracked, covering all 8 relevant source countries. 15 of 24 deals have an unconfirmed (RESEARCH_NEEDED) dollar value -- deal values for chip-export authorizations and telecom-vendor MoUs are rarely publicly disclosed, so Bahrain/Kuwait/Oman each have 2 tracked deals but still show an insufficient-data Capital Alignment Ratio (no disclosed value on either side). Three deals (HUMAIN's own launch, Stargate UAE, and the AirTrunk/Blackstone data-center JV) are same-country domestic buildouts, tracked but excluded from cross-border flow metrics -- see src/investment_flow_engine.py's own docstring for why. Not all deals are publicly disclosed; this is a curated sample, not an exhaustive record.",
